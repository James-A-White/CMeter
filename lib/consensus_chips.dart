...
class ChipsController extends GetxController {
  ...
  List<ChipModel> chipArray = <ChipModel>[];
  RxList<RxList<ChipModel>> rowArray = <RxList<ChipModel>>[].obs;
  ...
  /// chips is maximum.
  void updateChips({List<ChipModel>? chips}) {
    ...
    while (true) {
      ...
      print('${chipArray.length}($r): $a1 - $a2');
    ...
    int maxChipsInRow = (chipArray.length / r).ceil();
    ...
    for (int i = 0; i < r; i++) {
      RxList<ChipModel> row = <ChipModel>[].obs;

      for (int j = 0; j < maxChipsInRow; j++) {
        if (((i * maxChipsInRow) + j) >= chipArray.length) {
          stopProcessing = true;
          break;
        }
        row.add(chipArray[(i * maxChipsInRow) + j]);
      }
  ...
  num _calculateOverallChipArea(num h, num w, int r) {
    ...
    num totalArea = (minSideLength * minSideLength) * chipArray.length;
          ...
          (index) => Row(
            ...
            children: List.generate(
              ...
              (index2) => Container(
                ...
                decoration: BoxDecoration(border: Border.all(color: Colors.white), color: HexColor(c.rowArray[index][index2].chipColor)),
                child: Center(
                  child: Column(
                    children: [
                      Expanded(
                        ...
                        child: Padding(
                          ...
                          child: Image.asset('images/numbers/${c.rowArray[index][index2].opinionRating}.png'),
                        ),
                      ),
                      Expanded(
                        flex: 40,
                        child: Container(
                          ...
                          child: AutoSizeText(
                            c.rowArray[index][index2].chipText,
                            maxFontSize: 500,
                            minFontSize: 10,
                            ...
                            style: TextStyle(
                              ...
                              fontSize: 500,
                              color: HexColor(c.rowArray[index][index2].textColor),
                            ),
                          ),
                        ),
                      ),
